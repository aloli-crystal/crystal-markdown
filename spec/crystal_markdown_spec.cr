...
describe CrystalMarkdown do
  describe ".to_asciidoc" do
    it "converts a heading" do
      result = CrystalMarkdown.to_asciidoc("# Hello")
      result.should contain("= Hello")
    end

    it "converts bold text" do
      result = CrystalMarkdown.to_asciidoc("This is **bold** text.")
      result.should contain("*bold*")
    end

    it "converts italic text" do
      result = CrystalMarkdown.to_asciidoc("This is *italic* text.")
      result.should contain("_italic_")
    end

    it "converts inline code" do
      result = CrystalMarkdown.to_asciidoc("Use `puts` to print.")
      result.should contain("`puts`")
    end

    it "converts a code block" do
      md = "```crystal\nputs \"hello\"\n```"
      result = CrystalMarkdown.to_asciidoc(md)
    ...
    it "converts a horizontal rule" do
      result = CrystalMarkdown.to_asciidoc("---")
  ...
  describe ".to_html" do
    ...
    it "converts a code block to HTML" do
      md = "```crystal\nputs \"hi\"\n```"
      result = CrystalMarkdown.to_html(md)
  ...
  describe ".to_html_standalone" do
    it "returns a full HTML document" do
      result = CrystalMarkdown.to_html_standalone("# Title\n\nContent.")
  ...
  describe "VERSION" do
    it "has a version string" do
      CrystalMarkdown::VERSION.should_not be_empty
    end
  end

  describe ".sample_markdown" do
    it "returns a non-empty sample document" do
      sample = CrystalMarkdown.sample_markdown
    ...
    it "converts sample to AsciiDoc without error" do
      sample = CrystalMarkdown.sample_markdown
      result = CrystalMarkdown.to_asciidoc(sample)
      result.should_not be_empty
    end

    it "converts sample to HTML without error" do
      sample = CrystalMarkdown.sample_markdown
      result = CrystalMarkdown.to_html(sample)
      result.should_not be_empty
    end
  end
end
